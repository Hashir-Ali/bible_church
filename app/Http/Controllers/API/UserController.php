...
class UserController extends Controller
{
    ...
    {
    //    return $key = config('constants.FCM_SERVER_KEY');
        $user = auth()->user();
    ...
    public function profileUpdate(Request $request) 
    {
        ...
        {   
            ...
            $user->church_id = $request->church_id ?? $user->church_id;
        ...
        if($otherUser)
        {
            ...
            foreach($postDetail as $post){
                ...
                $postArr = [
                    ...
                    'pic' => $post->user->pic, 
                    'church_name' => ($post->user->churchData->church_name ?? null),
                    'comments_count' => $post->postComment->count()
                ];
    ...
    /* updateUserChurch **/
    public function userChurchUpdate(Request $request)
    {
        DB::beginTransaction();
        try
        {
            $churchWebsite = $request->church_website ?? null;
            $churchName = $request->church_name ?? null;

            $user = auth()->user();
//            pr($user->toArray());
            if(!empty($churchName)){
                $user->church_name = $churchName;
                $userChurch = Church::where('church_name', $churchName)->where('user_id', $user->id)->first();
                if($userChurch)
                {
                    return response([
                        'success' => false,
                        'message' => "Church name is already exist for this user!",
                        'data' => Config('constants.emptyData'),
                    ], config('constants.invalidResponse.statusCode'));
                }
                ...
                try {
                    $church = Church::create($churchInsertData);
                } catch (Exception $e) {
                    return response([
                        'success' => false,
            ...
            if(!empty($churchWebsite))
            $user->church_website = $churchWebsite;
            ...
            return response([
                'success' => true,
                'message' => 'User Church Details Updated',
                'data' => config('constants.emptyData')
            ], config('constants.validResponse.statusCode')); 
        } catch (\Throwable $e) {
            DB::rollBack();
