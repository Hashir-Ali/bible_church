...
class FriendController extends Controller
{
    ...
    public function suggestionList(Request $request)
    {
        ...
        $church_member->where('church_id', auth()->user()->church_id);
        ...
        foreach($church_member as $member){
            ...
            if($member->id != auth()->user()->id && $member->church_id == auth()->user()->church_id){
        ...
        $group_chat = GroupChat::query();
        $group_chat = $group_chat->get();
//      echo auth()->user()->id;exit;
        $group_chats = [];
        foreach($group_chat as $grp_chat){
            $members_ids_arr = !empty($grp_chat->members_id) ? json_decode($grp_chat->members_id) : null;
            if(in_array(auth()->user()->id,$members_ids_arr)){
                array_push($group_chats, $grp_chat);
            }
        }
        ...
        return response([
            'success' => true,
            'message' => 'success',
            'data' => ['church_members'=>$churchMember,'friends'=>$friends,'group_chats'=>$group_chats],
    ...
    public function sendRequest(Request $request){
        ...
        try {
            ...
            $church_member = User::query();
            $church_member->whereNotIn('church_id',[0]);
            $church_member->where('church_id', $authUser->church_id);
...
//            }
    ...
    public function acceptRequest(Request $request){
        if(!Friend::where('user_id', auth()->user()->id)->where('friend_id', $request->friend_id)->first())
        {
            Friend::create([
            ...
            FriendRequest::where([['sender_id','=',$request->friend_id],['receiver_id','=',auth()->user()->id]])->delete();
    ...
    public function getFriendIdByUniqueID($uniqueID){
        $user = User::where('uniqid','=',$uniqueID)->first();
        ...
        if($user) {
            ...
            if(count($friends) > 0) {
                foreach ($friends as $key => $friend) {
                    $friends_id_arr[$key] = $friend->friend_id;
                }
            }

            return response([
                'success' => true,
                'message' => 'Friend ID Fetched',
                'data' => $friends_id_arr,
            ], config('constants.validResponse.statusCode'));
        }
        return dataNotFound();
