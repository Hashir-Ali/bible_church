...
class PostDataController extends Controller
{
    ...
    public function list(Request $request){
        ...
        foreach($postDetail as $post){
            ...
            $churchName = isset($post->user->churchData->church_name) ? $post->user->churchData->church_name : $post->user->church_name;
            $postArr = [
                'post_id'    => $post->id,
                'title'    => $post->title,
    ...
    public function homePostList(Request $request){
        ...
        $postDetail = Post::orderBy('updated_at', 'desc');
        $postDetail->whereHas('user', function($query) {
        ...
        $postDetail->with('user', function($user){
            $user->with('churchData');
        });
        $postDetail->with('postMedia');
        $postDetail->with('like', function($like){
            $like->where('user_id',auth()->user()->id);
        });
        ## fetch only user and his friends post
        $friendsList = Friend::where('user_id', auth()->user()->id)->pluck('friend_id');
        $friendsList->push(auth()->user()->id);
        $postDetail->whereIn('user_id', $friendsList);
    ...
    public function addLike(Request $request) {
        ...
        try {
            ...
            Like::create(['post_id' => $request->post_id,'user_id' => auth()->user()->id]);
            DB::commit();
