...
function church_name_helper($church_id)
{
    return Church::where('id', $church_id)->value('church_name');
}
...
function member($church_id, $mem_id)
{
    // return User::where([['id' ,'=', $mem_id], ['u_type', '=', config('constants.u_type.user') ]])->first();
    return User::where([['church_id', '=', $church_id],['id' ,'=', $mem_id], ['u_type', '=', config('constants.u_type.user') ]])->first();
}
...
/* print variable or array with pretty print*/
if(!function_exists('pr')){
    ...
    function pr($var,$exit=1)
    {
        ...
        {
            echo "PRETTY PRINT ARRAY BY VPN:";
            echo "\n\n";
